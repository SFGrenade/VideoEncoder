...
bool StartNewSequence( int32_t width, int32_t height ) {
  printInFile( "%s( width=%d, height=%d ) - MacOS", __FUNCTION__, width, height );
...
bool SendPngBytes( uint8_t const* bytes, int32_t length ) {
  printInFile( "%s( bytes: %p, length: %d ) - MacOS", __FUNCTION__, bytes, length );
...
bool StopSequence() {
  printInFile( "%s() - MacOS", __FUNCTION__ );
