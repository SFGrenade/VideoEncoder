...
bool StartNewSequence( int32_t width, int32_t height ) {
  printInFile( fmt::format( "{:s}( width={:d}, height={:d} ) - MacOS", __FUNCTION__, width, height ) );
...
bool SendPngBytes( uint8_t const* bytes, int32_t length ) {
  printInFile( fmt::format( "{:s}( bytes={:p}, length={:d} ) - MacOS", __FUNCTION__, static_cast< void const* >( bytes ), length ) );
...
bool StopSequence() {
  printInFile( fmt::format( "{:s}() - MacOS", __FUNCTION__ ) );
